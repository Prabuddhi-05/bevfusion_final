...
import sys, traceback # Added
from torch.cuda import memory_allocated, memory_reserved, max_memory_allocated # Added 
    ...
    def forward(self, input):
        ...
        if self.inverse:
            ...
            if self.indice_key is not None and datas is not None:
                ...
                # CUDA error 2  == cudaErrorMemoryAllocation  (and PyTorch sometimes says just "CUDA out of memory")
                    if "error 2" in str(e).lower() or "out of memory" in str(e).lower():
                        rank = (
                            torch.distributed.get_rank()
                            if torch.distributed.is_initialized()
                            else 0
                            )
                    if rank == 0:          # only print once if running distributed
                        print("\n==========  CUDA OOM DETECTED  ==========", file=sys.stderr)
                        print(f"allocated : {memory_allocated()  / 1024**2:9.1f} MB", file=sys.stderr)
                        print(f"reserved  : {memory_reserved()   / 1024**2:9.1f} MB", file=sys.stderr)
                        print(f"max alloc : {max_memory_allocated() / 1024**2:9.1f} MB", file=sys.stderr)
                        print("spatial_shape :", spatial_shape,               file=sys.stderr)
                        print("kernel_size   :", self.kernel_size,             file=sys.stderr)
                        print("stride        :", self.stride,                  file=sys.stderr)
                        print("padding       :", self.padding,                 file=sys.stderr)
                        print("==========================================\n", file=sys.stderr)
                        traceback.print_exc(file=sys.stderr)   # full Python stac
                        sys.stderr.flush()
                raise e        # re‑throw so the runner still aborts
