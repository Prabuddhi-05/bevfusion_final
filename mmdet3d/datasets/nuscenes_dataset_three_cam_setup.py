...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if self.modality["use_camera"]:
            ...
            keep_cameras = ['CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_FRONT_LEFT']
            #keep_cameras = ['CAM_FRONT', 'CAM_FRONT_RIGHT']
            
            for cam_name, camera_info in info["cams"].items():
                if cam_name not in keep_cameras:
                    continue  # Skip cameras not in the list
                data["image_paths"].append(camera_info["data_path"])
                
                print(f"\n==========  CAMERA COUNT CHECK  ==========", file=sys.stderr)
                print(f"Using {len(data['image_paths'])} cameras for sample {info['token']}", file=sys.stderr)
                #print(f"[DEBUG] Camera order: {list(info['cams'].keys())}", file=sys.stderr)
            ...
            #for _, camera_info in info["cams"].items():
                ...
                if "cam_intrinsic" in camera_info:
                    ...
                    raise KeyError("Both 'cam_intrinsic' and 'camera_intrinsics' are missing in camera_info.")
                assert camera_intrinsic_matrix.shape == (3, 3), \
                    f"Expected 3x3 intrinsics, but got {camera_intrinsic_matrix.shape}"
