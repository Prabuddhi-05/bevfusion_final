...
class AwareBEVDepth(BaseTransform):
    def __init__(
        ...
        use_points = 'lidar', 
        downsample: int = 1,  # Changed 1 to 2
        bevdepth_downsample: int = 8, # Changed 16 to 8
    ...
    def get_depth_loss(self, depth_labels, depth_preds):
        import sys
        print("DEBUG >> Inside get_depth_loss()", flush=True)
        sys.stdout.flush()
        ...
        # Reorder and select 3 cameras (CAM_FRONT, CAM_FRONT_RIGHT, CAM_FRONT_LEFT)
        keep_indices = [0, 1]  # Based on the order you printed earlier
        if depth_labels.shape[0] >= 6:
            depth_labels = depth_labels[keep_indices]
            depth_preds = depth_preds[keep_indices]
            print(f"DEBUG >> Filtered for 2 cameras. New depth_labels shape: {depth_labels.shape}", flush=True)
            print(f"DEBUG >> Filtered for 2 cameras. New depth_preds shape: {depth_preds.shape}", flush=True)

        # Expand depth_labels to match spatial dimensions (H, W) of depth_preds
        if depth_labels.dim() == 2:  # [N, D]
            B, D, H, W = depth_preds.shape
            depth_labels = depth_labels.unsqueeze(-1).unsqueeze(-1).expand(B, D, H, W)

        # Permute and flatten both predictions and labels
        depth_preds = depth_preds.permute(0, 2, 3, 1).contiguous().view(-1, self.depth_channels)
        depth_labels = depth_labels.permute(0, 2, 3, 1).contiguous().view(-1, self.depth_channels)
        
        fg_mask = torch.max(depth_labels, dim=1).values > 0.0
 
        # Additional shape debugging
        print(f"DEBUG >> depth_preds shape: {depth_preds.shape}", flush=True)
        print(f"DEBUG >> depth_labels shape: {depth_labels.shape}", flush=True)
        print(f"DEBUG >> fg_mask shape: {fg_mask.shape}", flush=True)
        sys.stdout.flush()
...
class AwareDBEVDepth(BaseDepthTransform):
    def __init__(
        ...
        use_points = 'lidar', 
        depth_input = 'scalar', 
        height_expand = False,  # Ignore - for radar
        downsample: int = 1,
